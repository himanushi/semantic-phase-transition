...
- **完全分解**: `σ(l, h) = h · f_max(word) · g(l/L)` — 語依存のスケール × 普遍浸透関数
- **g(l/L)**: 語・モデルサイズに依存しない普遍関数（語間相関>0.96, モデル間相関0.98）
- **g の構造**: l/L < 0.85 でほぼ線形 (g ≈ (l/L)^0.9) + 最終層でドロップ (unembedding 効果)
- **二段階構造**: 定速浸透 + unembedding 再構成ドロップ
...
| exp3 | 完了 | 線形応答の限界 + 普遍浸透関数 g(l/L) の決定 |
